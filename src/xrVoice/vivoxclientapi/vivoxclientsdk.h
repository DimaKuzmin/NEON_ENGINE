...
///
/// The Vivox Client SDK implements a "C" message passing interface. The developer is responsible for creating requests and issuing those requests to the SDK using the vx_issue_request2() method, and then retrieving event and response messages from the SDK using the vx_get_message() method. The developer is also required to maintain state based on those responses and events.
///
